        ...
        <div class="tab-pane active" id="tab-general">
            <div class="tab-pane active" id="tab-general">
                <div
                    ...
                    <div class="col-md-12">
                        <label class="control-label" for="language_id">{{
							trans("admin.language") }}</label> <select
                ...
                <div class="form-group {{{ $errors->has('name') ? 'has-error' : '' }}}">
                    <div class="col-md-12">
                        <label class="control-label" for="name"> {{trans("modal.title") }}</label> <input class="form-control" type="text" name="name" id="name" value="{{{ Input::old('name', isset($photo) ? $photo->name : null) }}}" />
                        {{$errors->first('name', '<span class="help-block">:message </span>')}}
                    </div>
                </div>
                <div class="form-group {{{ $errors->has('photo_album_id') ? 'error' : '' }}}">
                    <div class="col-md-12">
                        <label class="control-label" for="photo_album_id">{{trans("photo.album") }}</label> 
                        <select style="width: 100%" name="photo_album_id" id="photo_album_id" class="form-control"> 
                            ...
                            <option value="{{$item->id}}"
                                    ...
                                    selected="selected" @endif @endif >{{$item->name}}
                        </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group {{{ $errors->has('album_cover') ? 'has-error' : '' }}}">
                    <div class="col-lg-12">
                        <label class="control-label" for="album_cover">{{{
							trans('photo.album_cover') }}}</label> <label
							...
							'class'=>'radio')) !!} {{{ trans('admin.no') }}} </label>
                ...
                <div
                    ...
                    <div class="col-lg-12">
                        <label class="control-label" for="image">{{
							trans("photo.picture") }}</label> <input name="image"
