...
!function(e) {
    ...
    i.prototype = {
        ...
        _initTemplateDefaults: function() {
            ...
            A = '<iframe class="kv-preview-data file-preview-office" src="https://docs.google.com/gview?url={data}&embedded=true"' + O + "></iframe>",
        ...
        readFiles: function(i) {
            ...
            a = function(x) {
                ...
                x >= f)
                    ...
                    s.removeClass("file-thumb-loading"),
                    void d.html("");
                var T, E, S, k, F, P, I, A, z, D, $, j, U = v + x, B = u + "-" + U, O = i[x], R = m.text, L = m.image, M = m.html, Z = O.name ? r.slug(O.name) : "", N = (O.size || 0) / 1e3, H = "", V = t.objUrl.createObjectURL(O), W = 0, q = "", K = 0, Y = function() {
                    var e = p.setTokens({
                        index: x + 1,
                        files: f,
                        percent: 50,
                ...
                0 !== W) ? r.showPreview ? !C && N > b ? (r.addToStack(O),
                s.addClass("file-thumb-loading"),
                r._previewDefault(O, B),
                r._initFileActions(),
                r._updateFileDetails(f),
                void a(x + 1)) : (o.length && void 0 !== FileReader ? (z = R(O.type, Z),
                D = M(O.type, Z),
                $ = L(O.type, Z),
                ...
                ,
                z || D ? l.readAsText(O, r.textEncoding) : $ ? l.readAsDataURL(O) : l.readAsArrayBuffer(O)) : (r._previewDefault(O, B),
                setTimeout(function() {
                    a(x + 1),
                    r._updateFileDetails(f)
                }, 100),
                r._raise("fileloaded", [O, B, x, l])),
                void r.addToStack(O)) : (r.isAjaxUpload && r.addToStack(O),
                setTimeout(function() {
                    a(x + 1),
                    r._updateFileDetails(f)
                }, 100),
                void r._raise("fileloaded", [O, B, x, l])) : (S = r.msgInvalidFileExtension.setTokens({
                    name: Z,
                    extensions: _
                }),
                void y(S, O, B, x))
